    ...
    if process_this_frame:
        ...
        for face_encoding in face_encodings:
            ...
            if matches[best_match_index]:
                ...
                name = name.replace(
                    "/Users/matthewkayne/Documents/School/A-Levels/Computer Science/Project/Code/", "")
...
def storeLink(tempDeviceId, deviceState):
    database.cursor.execute(
        "SELECT id from link WHERE userid = ? AND deviceid = ?", (userId, tempDeviceId))
    if not database.cursor.fetchall():  # An empty result evaluates to False.
        database.cursor.execute(
            """INSERT INTO link (userid, deviceid, state) VALUES (?, ?, ?)""", (userId, tempDeviceId, deviceState))
    else:
        database.cursor.execute(
            """UPDATE link SET state=? WHERE userid=? AND deviceid=?""", (deviceState, userId, tempDeviceId))
...
def settings():
    ...
    light_1CheckButton = Checkbutton(settingsPage, text="Light On/Off", variable=light_1.state,
                                     command=lambda: storeLink(light_1.deviceid, light_1.state.get()))
    light_1CheckButton.pack()

    spotify = Device(2, BooleanVar())
    spotify.state.set(True)
    spotifyCheckButton = Checkbutton(settingsPage, text="Spotify On/Off", variable=spotify.state,
                                     command=lambda: storeLink(spotify.deviceid, spotify.state.get()))
    spotifyCheckButton.pack()

    database.cursor.execute("SELECT email FROM accounts WHERE username = ? AND password = ?",
                            (logInName.get(), myHash(logInPass.get())))
    email = database.cursor.fetchone()
    database.cursor.execute("SELECT username, email FROM accounts WHERE username = ? AND password = ?",
                            (logInName.get(), myHash(logInPass.get())))
    data = database.cursor.fetchall()

    dataButton = Button(settingsPage, text="Send My Private Data", command=lambda: [
                        sendData(email[0], str(data))])
    dataButton.pack(pady=5, side=BOTTOM)

    confirmButton = Button(settingsPage, text="Confirm",
...
def accountCheck(inName, inPass):
    ...
    database.cursor.execute(
        "SELECT username FROM accounts WHERE username = ? AND password = ?", (inName, inPass))
...
def log_in():
    ...
    button = Button(logInPage, text="Ok", command=lambda: [accountCheck(logInName.get(), myHash(
        logInPass.get())), logInPage.destroy()])  # Creates a Button Widget in the LogInPage Window
    button.pack(pady=5, side=TOP)
...
def faceControl():
    ...
    if name == None:
        ...
        for x in range(db):
            database.cursor.execute(
                "SELECT id FROM accounts WHERE filename = ?", ((name+".jpg").replace(" ", ""),))
            fcUserId = sum(database.cursor.fetchone())
            database.cursor.execute(
                "SELECT deviceid FROM link WHERE userid = ? AND id= ?", (fcUserId, x+1,))
            fcDeviceId = sum(database.cursor.fetchone())
            database.cursor.execute(
                "SELECT devicename FROM devices WHERE id = ?", (fcDeviceId,))
            fcDeviceName = database.cursor.fetchone()[0]
            database.cursor.execute(
                "SELECT state FROM link WHERE userid = ? AND deviceId = ?", (fcUserId, fcDeviceId))
            ...
            if fcDeviceId == 1 and fcState == 0:
                ...
                requests.post("https://maker.ifttt.com/trigger/"+fcDeviceName +
                              fcStateString+"/with/key/fZQacqZEzguEOUTC2dSECFBo0xcNEk4ofpmJJoy2yIg")
...
who_btn = tk.Button(window, text="Who am I?", height=2,
                    width=10, command=lambda: [run(), faceControl()])
who_btn.grid(row=300, column=90)

new_btn = tk.Button(window, text="New Person?", height=2,
                    width=10, command=new_person)
new_btn.grid(row=300, column=250)

login_btn = tk.Button(window, text="Log In", height=2,
                      width=10, command=log_in)
